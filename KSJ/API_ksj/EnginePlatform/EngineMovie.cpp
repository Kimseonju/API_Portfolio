...
EngineMovie* EngineMovie::MainMoviePlay = nullptr;

HRESULT EngineMovie::Play(const EngineString& Path)
{
    // 
    if (S_OK != MainMoviePlay->OpenFile(Path.c_str()))
    {
        EngineDebug::AssertMsg(L"무비파일 오픈에 실패했습니다.");
    }
...
HRESULT EngineMovie::Pause()
{
    ...
    {
        EngineDebug::AssertMsg("메인 무비 재생기가 존재하지 않습니다 윈도우를 만드세요");
    }

    return S_OK;
}
HRESULT EngineMovie::Stop()
{
    if (nullptr != MainMoviePlay)
    {
...
EngineMovie::EngineMovie(HWND hwnd) :
...
EngineMovie::~EngineMovie()
{
    TearDownGraph();
}

// Open a media file for playback.
HRESULT EngineMovie::OpenFile(PCWSTR pszFileName)
{
...
HRESULT EngineMovie::HandleGraphEvent(GraphEventFN pfnOnGraphEvent)
{
    if (!m_pEvent)
    {
        return E_UNEXPECTED;
...
HRESULT EngineMovie::MoviePause()
{
...
BOOL EngineMovie::HasVideo() const
{
...
HRESULT EngineMovie::UpdateVideoWindow(const LPRECT prc)
{
    if (m_pVideo)
    {
...
HRESULT EngineMovie::Repaint(HDC _DC)
{
    if (m_pVideo)
    {
...
HRESULT EngineMovie::DisplayModeChanged()
{
    if (m_pVideo)
    {
...
HRESULT EngineMovie::InitializeGraph()
{
    TearDownGraph();
...
void EngineMovie::TearDownGraph()
{
...
HRESULT EngineMovie::CreateVideoRenderer()
{
    HRESULT hr = E_FAIL;
...
HRESULT EngineMovie::RenderStreams(IBaseFilter* pSource)
{
