...
Item_Polyphemus::Item_Polyphemus() :
	m_Anim(nullptr),
	m_Basic(nullptr)
{
}
...
void Item_Polyphemus::Start()
{
	Item::Start();
	m_ItemRenderer->TransSetting(L"collectibles_polyphemus.bmp", { 64.f,64.f });
	m_ItemRenderer->SetPivotPos({ 0.f, -40.f });
}

void Item_Polyphemus::PlayerFarming()
{
	...
	player->GetCharacterInfo()->AddAtt(5);
...
EngineRendererBasic* Item_Polyphemus::PlayerRenderItem()
{
	if (nullptr == LogicValue::MainPlayer)
		EngineDebug::AssertMsg("if (nullptr == LogicValue::MainPlayer)");
	...
	m_Basic->TransSetting(L"collectibles_polyphemus.bmp", { 64.f,64.f });
