...
Item_thesadonion::Item_thesadonion()
	:m_Anim(nullptr)
	, m_Basic(nullptr)
{
}
...
void Item_thesadonion::Start()
{
	Item::Start();
	m_ItemRenderer->TransSetting(L"collectibles_001_thesadonion.bmp", { 64.f,64.f });
	m_ItemRenderer->SetPivotPos({ 0.f, -40.f });
}

void Item_thesadonion::PlayerFarming()
{
	...
	Costume_SadOnion* ItemCostume = player->GetScene()->CreateActor<Costume_SadOnion>();
...
EngineRendererBasic* Item_thesadonion::PlayerRenderItem()
{
	if (nullptr == LogicValue::MainPlayer)
		EngineDebug::AssertMsg("if (nullptr == LogicValue::MainPlayer)");
	...
	m_Basic->TransSetting(L"collectibles_001_thesadonion.bmp", { 64.f,64.f });
